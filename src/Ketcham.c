...
/* Correct each time interval for length reduction */
    ...
    *redDensity += AgeCorrectionKet(midLength);
  }
  *ftModelAge += AgeCorrectionKet(redLength[numTTNodes-2])*(tTPath[node].time-tTPath[node+1].time);
  *redDensity += AgeCorrectionKet(redLength[numTTNodes-2]); 
  ...
  *ftModelAge /= (stdLengthReduction*SECS_PER_MA);
  //printf("density = %10.3f \n", *redDensity / (numTTNodes-2)); 
}
  ...
  if (annealModel == KETCHAM_ET_AL) {
    if (InterpolateTTPathKet(numTTDefs,tTDef,&numTTNodes,tTPath,pctPerTimeStep)) {
      ...
      CalcModelAgesKet(&tTPath[0],redLength,numTTNodes,firstTTNode,oldestModelAge,
							 ftModelAge,stdLengthReduction,redDensity);
...
void ketch_main_(int *ntime,float ketchtime[],float ketchtemp[],double *alo,double *final_age,double *oldest_age,double *fmean,double fdist[], double *redDensity)
{
  ...
  lengthred = 0.893;
  annealmodel = KETCHAM_ET_AL; 
  kinpar = ETCH_PIT_LENGTH; //Dpar
  kinetic_par=1.65; //Dpar in microns
  l0model = L0_FROM_USER;

  doProject = 0;
  usedCf = 0;

  ForwardModel(ktime,lengthred,kinetic_par,(double)1.0,annealmodel,doProject,usedCf,kinpar,l0model,*alo,cdf,numPDFPts,pdfAxis,pdf,&oldestModelAge,&ftModelAge,&numPopulations,kerryTt,&redtrackdens);
